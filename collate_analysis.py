...
def main():
    ...
    for folder_id in range(1, 9375+1):
        ...
        pt_filepath = os.path.join(folder_path, "persistence_times.csv")
        ...
        pt_data = pd.read_csv(pt_filepath, index_col=0)
        particle_dataframe = copy.deepcopy(
            gridseach_dataframe[gridseach_dataframe["array_id"] == folder_id]
        )
        particle_dataframe = pd.concat(
            [particle_dataframe] * speed_data.shape[0], ignore_index=True
        )
        particle_dataframe["particle_rmsd"] = speed_data
        particle_dataframe["particle_pt"] = pt_data
