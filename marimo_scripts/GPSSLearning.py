...
def _(experiment_dataframe, plt, sns):
    _fig, _axs = plt.subplots(3, 1, figsize=(4, 6))
...
@app.cell
def _(ApproximateGPModel, gpytorch, torch):
...
@app.cell
def _():
    # Define which cell type we are currently fitting to:
    cell_index = 0
    return (cell_index,)
...
def _(
    ...
    speed_loss_hist = train_model(
        speed_model, speed_likelihood,
        normalised_parameters, speed_class_dataset,
        epochs=100
    )
    return (speed_loss_hist,)


@app.cell
def _(cf_model, speed_model):
...
def _(TD_GRIDSEARCH_PARAMETERS, qmc):
    ...
    oos_sample_matrix = oos_sampler.random_base2(m=19)
...
def _(
    ...
    return oos_cf_predictions, oos_speed_predictions


@app.cell
def _(bounded_gridsearch, oos_cf_predictions, oos_speed_predictions, plt):
    ...
    for _i in range(12):
        _axs[_i].hist(
            bounded_gridsearch[oos_speed_predictions > 0.5, _i],
            bins=50, range=(0, 1), density=True, alpha=0.5
        );
...
@app.cell
def _(np, oos_cf_predictions, oos_speed_predictions):
    joint_mask = np.logical_and(
        oos_cf_predictions > 0.5,
        oos_speed_predictions > 0.5
    )
...
def _(bounded_gridsearch, joint_mask, print_parameter_set):
    test_set = bounded_gridsearch[joint_mask, :][1]
    _ = print_parameter_set(test_set)
    return (test_set,)


@app.cell
def _(bounded_gridsearch, joint_mask, plt):
    _fig, _axs = plt.subplots(12, 1, figsize=(4, 10))

    for _i in range(12):
        _axs[_i].hist(
            bounded_gridsearch[joint_mask, _i],
